"""
Seeds the database with roles, demo users, sample patient records, synthetic
historical audit-log activity (so behavioral baselines exist immediately),
and trains + saves the two ML artifacts used by the app:

  1. IsolationForest  -> unsupervised anomaly detection (risk_engine.py)
  2. RandomForestClassifier -> SHAP surrogate for explainability (explainability.py)
...
from risk_engine import FEATURE_NAMES, rule_based_score
...
def seed_synthetic_audit_history():
    """Generates ~30 days of plausible normal-usage audit events per clinical
    user so BehaviorProfile has a real baseline to compare against as soon
    as the app starts (build_or_refresh_profile needs >=10 events, and
    evaluate_session_deviation wants a much larger sample before it will
    actively flag anything).
    """
    ...
    for user in clinical_users:
        # simulate 30 days x a handful of workday sessions x a few actions each
        for day in range(30, 0, -1):
            day_dt = now - timedelta(days=day)
            if day_dt.weekday() >= 5:  # skip weekends -> realistic office pattern
                continue
...
def _independent_synthetic_row():
    """Samples each feature independently (rather than only ever varying
    them in lockstep) so the surrogate model learns a clean, monotonic,
    per-feature effect. This keeps SHAP attributions intuitive: each
    feature's contribution should line up with what actually drove the
    rule-based score, which is what an admin reading the explanation expects.
    """
    return {
        "is_new_ip": random.choice([0, 0, 0, 1]),
        "is_off_hours": random.choice([0, 0, 0, 1]),
        "recent_failed_attempts": random.choice([0, 0, 0, 1, 2, 3, 4, 5]),
        "is_new_device": random.choice([0, 0, 0, 1]),
        "rapid_access_count": random.choice([0, 0, 1, 2, 3, 5, 8, 10, 12, 15]),
        "behavior_deviation_score": random.choice(
            [0, 5, 10, 15, 20, 30, 40, 50, 60, 70, 80, 90, 100]
        ),
    }


def train_ml_models(app):
    os.makedirs(app.config["ML_MODEL_DIR"], exist_ok=True)

    rows = [_independent_synthetic_row() for _ in range(3000)]
    X = np.array([[r[name] for name in FEATURE_NAMES] for r in rows], dtype=float)

    # Pseudo-labels come directly from the rule-based score, so the
    # surrogate model (and therefore its SHAP attributions) learns to
    # reproduce the same reasoning the risk engine's rules already encode -
    # each feature's marginal effect stays intuitive and directionally
    # correct instead of picking up spurious correlations from a small
    # hand-crafted sample.
    scores = np.array([rule_based_score(r) for r in rows])
    y = (scores >= 50).astype(int)

    # 1) IsolationForest - trained on the "normal" (low rule-score) slice,
    #    unsupervised, to catch anomaly patterns the hand-written rules
    #    didn't anticipate.
    normal_X = X[y == 0]
    iso = IsolationForest(n_estimators=150, contamination=0.1, random_state=42)
    iso.fit(normal_X)
    joblib.dump(iso, os.path.join(app.config["ML_MODEL_DIR"], "isolation_forest.joblib"))

    # 2) RandomForest surrogate model for SHAP explainability
    rf = RandomForestClassifier(n_estimators=300, max_depth=5, random_state=42)
    rf.fit(X, y)
    joblib.dump(rf, os.path.join(app.config["ML_MODEL_DIR"], "shap_surrogate_rf.joblib"))

    print(f"Trained on {len(rows)} synthetic samples ({y.sum()} positive / {len(y)-y.sum()} negative).")
    print("Saved IsolationForest + SHAP surrogate RandomForest to", app.config["ML_MODEL_DIR"])
